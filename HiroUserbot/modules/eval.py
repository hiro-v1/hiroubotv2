...
from HiroUserbot.config import DEVS
from HiroUserbot.core.function.emoji import emoji
import psutil

async def restart():
    execvp(executable, [executable, "-m", "HIROUserbot"])

async def cb_restart(client, callback_query):
    await callback_query.message.delete()
    os.system(f"kill -9 {os.getpid()} && python3 -m HIROUserbot")


async def cb_gitpull(client, callback_query):
    await callback_query.message.delete()
    os.system(f"kill -9 {os.getpid()} && git pull && python3 -m HIROUserbot")
